...
updateAlarmButton();
els.taskProgress.addEventListener("input", updateProgressLabel);
...
function loadTasks() {
  ...
  return [
    sampleTask("Finalisasi checklist dokumen PBG", "PBG Villa Maharani", "Wahyu", today, "high", "doing", 65, "Checklist struktur dan arsitektur sudah dicek, MEP masih menunggu revisi."),
    sampleTask("Follow up approval invoice termin", "Administrasi Mandala", "Ayu", today, "medium", "review", 85, "Draft pesan dan invoice sudah siap, menunggu konfirmasi nominal."),
    sampleTask("Survey kebutuhan gambar as-built", "SLF Apotek BIA", "Made", tomorrow, "medium", "todo", 10, "Jadwal survey sudah dikonfirmasi dengan pihak lokasi."),
    sampleTask("Rekap progress mingguan untuk owner", "PBG Villa Maharani", "Indra", yesterday, "high", "doing", 45, "Data dari tim gambar sudah masuk sebagian."),
    sampleTask("Upload backup file proyek ke drive", "Administrasi Mandala", "Ayu", today, "low", "done", 100, "Backup dokumen selesai diunggah."),
  ];
}

function sampleTask(title, project, assignee, dueDate, priority, status, progress, updateText) {
  return {
    id: createId(),
    title,
    project,
    ...
    progress,
    notes: "",
    updates: [{ date: todayInput(), text: updateText, progress }],
    reminderAt: "",
    alarmFiredAt: "",
...
function saveTaskFromForm() {
  ...
  const progress = clampProgress(els.taskProgress.value);
  const status = progress === 100 ? "done" : els.taskStatus.value;
...
function updateStatus(id, status) {
  ...
  task.status = status;
  if (status === "doing" && task.progress < 10) task.progress = 10;
  if (status === "review" && task.progress < 70) task.progress = 70;
...
function addUpdate(task, text) {
  ...
  task.updates = [
    ...(task.updates || []),
    { date: state.focusDate, text, progress: task.progress },
  ];
}
...
function renderSummary() {
  ...
  const average = total ? Math.round(state.tasks.reduce((sum, task) => sum + clampProgress(task.progress), 0) / total) : 0;

  els.totalCount.textContent = total;
  els.activeCount.textContent = active;
  els.doneCount.textContent = done;
  els.overdueCount.textContent = overdue;
  els.averageProgress.textContent = `${average}%`;
}
  ...
  return `
    <tr class="${rowClass}">
      ...
      <td class="progress-cell">
        <div class="progress-bar"><span style="--value: ${clampProgress(task.progress)}%"></span></div>
        <span class="progress-text">${clampProgress(task.progress)}%</span>
      </td>
...
function getFilteredTasks() {
  return [...state.tasks]
    .filter((task) => {
      const search = `${task.title} ${task.project} ${task.assignee} ${task.notes}`.toLowerCase();
...
function updateProgressLabel() {
  els.progressValue.textContent = `${clampProgress(els.taskProgress.value)}%`;
}
